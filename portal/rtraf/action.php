<?php
	...
	if ($_GET['action']=="delfolder"){
		  $sql = 'delete from rtraf_carpetas where id_carpeta="'.$_GET['data'].'"';		  
  		  $res = mysqli_query($con,$sql);
	}

	if ($_GET['action']=="addfolder"){
		  $sql = 'insert into rtraf_carpetas (descripcion) values ("'.$_GET['data'].'")';		  
  		  $res = mysqli_query($con,$sql);
	}

	if ($_GET['action']=="delrtraf"){
		  $sql = 'delete from rtraf where id_rtraf="'.$_GET['data'].'"';		  
  		  $res = mysqli_query($con,$sql);
	}
	

	if ($_GET['action']=="addrtraf"){
		...
		for ($i = 0; $i < sizeof($files['name']); $i++){
			...
			$sql = 'insert into rtraf (descripcion,id_carpeta,ruta) values ("'.$_POST['data'].'","'.$_POST['data2'].'","./archivos/'.$newname.'")';		  
			$res = mysqli_query($con,$sql);		  	
